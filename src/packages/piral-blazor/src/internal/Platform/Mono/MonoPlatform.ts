...
export const monoPlatform: Platform = {
  ...
  callEntryPoint: function callEntryPoint(assemblyName: string) {
    ...
    const invokeEntrypoint = bindStaticMethod(
      ...
      'Microsoft.AspNetCore.Blazor.Hosting.EntrypointInvoker',
      'InvokeEntrypoint',
    );
    ...
    loadAssemblyUrls.forEach(url => {
      ...
      asyncLoad(url).then(
        ...
        errorInfo => {
          ...
          const isPdb404 = errorInfo instanceof XMLHttpRequest && errorInfo.status === 404 && filename.match(/\.pdb$/);
          if (!isPdb404) {
            onError(errorInfo);
          }
          removeRunDependency(runDependencyId);
