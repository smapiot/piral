...
// https://github.com/NekR/offline-plugin/blob/master/src/misc/async-waituntil.js
// - and -
// https://github.com/NekR/offline-plugin/blob/master/src/misc/sw-template.js
...
function run(params, helpers, debug = false) {
  ...
  function matchCacheMap(request) {
    ...
    for (let i = 0; i < cacheMaps.length; i++) {
      ...
      let newString;
      const isSameHost = url.host === location.host;
      const requireSameHost = map.sameHost === true;

      if (typeof map.match === 'function') {
        newString = map.match(url, request);
      } else if (requireSameHost && !isSameHost) {
        newString = url;
      } else if (isSameHost && Array.isArray(map.exclusions) && map.exclusions.indexOf(url.pathname) !== -1) {
        newString = url;
      } else if (requireSameHost && typeof map.fallback === 'string') {
        newString = new URL(map.fallback, location);
      } else {
...
run(__PARAMS__, __HELPERS__, __DEBUG__);
