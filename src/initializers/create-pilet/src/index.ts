...
import { allCommands } from 'piral-cli';
...
const [command] = allCommands.filter(m => m.name === 'new-pilet');
const instructions = getCommandData(command.flags);
const questions = instructions.map(instruction => ({
...
prompt(questions).then(answers => {
  ...
  for (const instruction of instructions) {
    ...
    parameters[instruction.name] = getValue(instruction.type, value as any);
  }
