...
- **Folder structure:** Since we don't want to mix rush's files with my packages we add a folder "packages" in the root directory by running `mkdir tools`. All pilets and the app shell will live in their own folders directly inside the packages folder. In this scenario they are nested in a folder depth of 2 which is Rush's default. If you want do have a different structure you might need to configure `projectFolderMinDepth` and `projectFolderMaxDepth` inside *rush.json*. I'll just uncomment them being set to 2.
...
The version should match the version inside `packages\app-shell\package.json`. This way Rush can find the app-shell package inside the monorepo and will just symlink the dependency to `packages\app-shell`.
